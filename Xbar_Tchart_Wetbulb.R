...
#### Nasty wetbulb calculations ####
wetbulb<-merge(tempdat, humdat, by=c("room", "Label"), na.rm=TRUE)
wetbulb$Temp<-(wetbulb$value.x-32)/1.8

wetbulb$wbtemp<-wetbulb$Temp*atan(0.151977*(sqrt(wetbulb$value.y+8.313659)))+atan(wetbulb$Temp+wetbulb$value.y)-atan(wetbulb$value.y-1.676331)+
                0.00391838*(wetbulb$value.y^(3/2))*atan(0.023101*wetbulb$value.y)-4.686035

wetbulb<-wetbulb[complete.cases(wetbulb),]

#ggplot(data = wetbulb, aes(x=wetbulb$Label, y=wetbulb$wbtemp, color=wetbulb$Room)) + geom_line()

### X-bar and R Charts for wetbulb sets ###
##### Initial code set for one pod ######
wbsub <- subset(wetbulb, room == "Pod_3")
#wbsub <- wetbulb[complete.cases(wetbulb),]
wbset<-qcc.groups(wbsub$wbtemp, wbsub$Label)
  qcc(data= wbset, type="R", nsigmas=3, sizes=8)
  qcc(data= wbset, type="xbar", nsigmas=3, sizes=12)
